...
function getParentCategories($cat, $owner_uid)
{
    ...
    $result = db_query(
        "SELECT parent_cat FROM ttrss_feed_categories
...
function queryFeedHeadlines(
    ...
    if ($filter) {
        ...
        $result = db_query(
            ...
            WHERE $filter_query_part LIMIT 1",
            false
        );
...
function format_article($id, $mark_as_read = true, $zoom_mode = false, $owner_uid = false)
{
    ...
    $result = db_query(
        "SELECT id,title,link,content,feed_id,comments,int_id,lang,
...
function load_filters($feed_id, $owner_uid, $action_id = false)
{
    ...
    $result = db_query(
        "SELECT * FROM ttrss_filters2 WHERE
...
function get_feeds_from_html($url, $content)
{
    ...
    $xpath = new DOMXPath($doc);
    $entries = $xpath->query(
        '/html/head/link[@rel="alternate" and '.
